...
class Prepare_dataset:
    ...
    def __init__(self, batch_size = 32, shape = '1D', dtype='train'):
        ...
        reshape = [IMAGE_SIZE * IMAGE_SIZE] if (shape == '1D') else [IMAGE_SIZE, IMAGE_SIZE]

        self.train_data = extract_data(train_data_filename, 60000, shape = reshape)
        self.train_labels = extract_labels(train_labels_filename, 60000)
        self.test_data = extract_data(test_data_filename, 10000, shape = reshape)
    ...
    def __call__(self, step = 0, dtype = 'train'):
        if (dtype=='train'):
