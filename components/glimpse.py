...
class GlimpsNetwork(object):
    ...
    def __call__(self, loc_tensor, guess):
        with tf.variable_scope(self.scope or 'Glimpse', reuse = tf.AUTO_REUSE) as scope:
            ...
            # Glimps network upper part
            self.guess_net = tl.layers.InputLayer(guess, name='guess_part_input_layer')
            self.guess_net = tl.layers.DenseLayer(self.guess_net, n_units = 128, name ='guess_fc_1')
            self.guess_net = tl.layers.BatchNormLayer(self.guess_net, name='guess_bn_1')
            self.guess_net = tf.nn.relu(self.guess_net.outputs, name='guess_relu_1')

            # Glimps network right part
            self.glimps_net = tf.concat([self.retina_imgs, self.location_net, self.guess_net], axis=-1)       
