...
class GlimpsNetwork(object):
    ...
    def __call__(self, loc_tensor):
        with tf.variable_scope(self.scope or 'Glimpse', reuse = tf.AUTO_REUSE) as scope:
            ...
            self.location_net = tl.layers.DenseLayer(self.location_net, n_units = 128, name ='ind_fc_2')
            self.location_net = tl.layers.BatchNormLayer(self.location_net, name='ind_bn_2')
            self.location_net = tf.nn.relu(self.location_net.outputs, name='ind_relu_2')
